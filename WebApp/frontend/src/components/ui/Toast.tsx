...
function ToastItem({ toast, onClose }: { toast: Toast; onClose: () => void }) {
  ...
  const colors = {
    ...
    info: 'bg-zinc-900/80 border-zinc-700/50 text-zinc-300',
  ...
  return (
    <motion.div
      ...
      className={`pointer-events-auto w-full rounded-lg border backdrop-blur-sm ${colors[toast.type]}`}
    >
      <div className="flex items-start gap-3 p-4">
        <span className={iconsColor[toast.type]}>{icons[toast.type]}</span>
